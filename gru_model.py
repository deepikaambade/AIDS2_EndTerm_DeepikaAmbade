...
def train_model(model, train_loader, epochs=50, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        for x_batch, y_batch in train_loader:
            ...
            # Reshape for GRU input [batch, time_steps, features]
    ...
    plt.savefig('results/gru_training_loss.png')
    plt.close()
...
def plot_waveform_comparison(original, predicted):
    ...
    plt.savefig('results/gru_waveform_comparison.png')
    plt.close()
...
def plot_spectrogram_comparison(original, predicted):
    try:
        ...
        plt.colorbar(format='%+2.0f dB')
        plt.title('GRU Predicted Mel Spectrogram')
        ...
        plt.savefig('results/gru_spectrogram_comparison.png')
        plt.close()
...
def main():
    ...
    gru_prediction = generate_prediction(gru_model, input_audio, input_frames, output_frames)
    ...
    prediction_file = 'results/gru_prediction.mp3'
    sf.write(prediction_file, gru_prediction, SAMPLE_RATE)
    ...
    # Calculate MSE
    min_len = min(len(original_next), len(gru_prediction))
    gru_mse = np.mean((original_next[:min_len] - gru_prediction[:min_len])**2)
    print(f"GRU MSE: {gru_mse:.6f}")

    # Create visualizations
    print("Creating visualizations...")
    plot_waveform_comparison(original_next, gru_prediction)
    plot_spectrogram_comparison(original_next, gru_prediction)
