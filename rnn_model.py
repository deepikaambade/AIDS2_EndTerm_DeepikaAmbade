...
def plot_waveform_comparison(original, predicted):
    ...
    plt.savefig('results/rnn_waveform_comparison.png')
    plt.close()
...
def plot_spectrogram_comparison(original, predicted):
    try:
        ...
        plt.colorbar(format='%+2.0f dB')
        plt.title('RNN Predicted Mel Spectrogram')
        ...
        plt.savefig('results/rnn_spectrogram_comparison.png')
        plt.close()
...
def main():
    ...
    rnn_model = VanillaRNN(input_dim, hidden_dim, output_dim)
    ...
    rnn_prediction = generate_prediction(rnn_model, input_audio, input_frames, output_frames)
    ...
    prediction_file = 'results/rnn_prediction.mp3'
    sf.write(prediction_file, rnn_prediction, SAMPLE_RATE)
    ...
    # Calculate MSE
    min_len = min(len(original_next), len(rnn_prediction))
    rnn_mse = np.mean((original_next[:min_len] - rnn_prediction[:min_len])**2)
    print(f"RNN MSE: {rnn_mse:.6f}")

    # Create visualizations
    print("Creating visualizations...")
    plot_waveform_comparison(original_next, rnn_prediction)
    plot_spectrogram_comparison(original_next, rnn_prediction)
